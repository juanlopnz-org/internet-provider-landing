...
export default function Hero({ config, theme, contact }: HeroProps) {
    ...
    <section id='inicio' className="min-h-[calc(100vh-10rem)] md:min-h-[calc(100vh)] flex items-center justify-center overflow-hidden bg-mesh md:bg-mesh-md lg:bg-mesh-lg relative pt-40 pb-20 md:p-20">
      <div className="container-custom relative z-10">
        <div className="max-w-5xl mx-auto text-center">
          ...
          >
            <span className="block text-dark">{config.title}</span>
          ...
          >
            <a href={config.ctaButtons.primary.link} className="btn btn-primary text-lg">
              {config.ctaButtons.primary.text}
              <svg className="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
              </svg>
            </a>
            <a href={config.ctaButtons.secondary.link} className="btn btn-secondary text-lg">
              {config.ctaButtons.secondary.text}
            </a>
            <a
              href={config.ctaButtons.tertiary.link}
              target="_blank"
              rel="noopener noreferrer"
              className="btn text-lg bg-green-500 text-white hover:bg-green-600 hover:scale-105"
            >
              ...
              {config.ctaButtons.tertiary.text}
            </a>
          </div>
          ...
          >
            {config.stats.map((stat, index) => (
              <div
                key={index}
                className="card backdrop-blur-sm bg-white/80 hover:bg-white transform hover:-translate-y-2"
                style={{ animationDelay: `${600 + index * 100}ms` }}
              >
                <div className="text-4xl md:text-5xl font-bold text-gradient mb-2">{stat.label}</div>
                <div className="text-lg font-semibold text-dark mb-1">{stat.value}</div>
